...
1. Multi-select threads from the threads table. On the **Threads** tab, multi-select threads. At the bottom of the page, click <Icon icon="database" /> **Add to Dataset**. You can add at most **100** threads in a single action.
    ...
    <Warning>
    Because the dataset is created before the threads are added, a failure at the second step leaves the new dataset in place and empty. LangSmith reports this as **Dataset created, but the threads were not added** and links the dataset so you can retry the add or delete it.
    </Warning>
...
<Note>
The picker lists every existing dataset in the workspace, including datasets whose [schema](#create-a-dataset-schema) a conversation does not satisfy. Incompatible datasets are not filtered out, so the add reports an error instead. Choose a dataset that has no schema, or one whose schema accepts a conversation.
</Note>

### Automatically from a tracing project

You can use [automation rules](/langsmith/rules) to add traces automatically to a dataset based on certain conditions. For example, you could add all traces that are [tagged](/langsmith/observability-concepts#tags) with a specific use case or have a [low feedback score](/langsmith/observability-concepts#feedback).

A rule's [item type](/langsmith/rules#set-the-item-type-to-runs-or-threads) determines what it adds. A rule whose item type is **Runs** adds one example per matching trace. A rule whose item type is **Threads** waits until the conversation goes idle, then adds one example per matching thread.
...
You can optionally configure annotation queues with a default dataset, though you can add runs and threads to any dataset by using the dataset switcher. Once you select the right dataset, click **Add to Dataset** or hit the hot key `D` to add the item to it. A run item adds that run, and a thread item adds the whole conversation as one example.

Any modifications you make to a run in your annotation queue will carry over to the dataset, and all metadata associated with the run will also be copied. Edits apply to run items only: a thread item is added as it was traced.

<Note>
Thread items do not offer support for default datasets.
</Note>

<Tip>
You can also set up rules to add runs or threads that meet specific criteria to an annotation queue using [automation rules](/langsmith/rules).
</Tip>
