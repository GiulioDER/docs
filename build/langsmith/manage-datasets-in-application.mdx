...
- **One example per thread**: Each thread becomes a single example containing the full conversation, rather than one example per run within the thread.
- **No reference outputs**: A thread example is only saved as an input with no reference output.
