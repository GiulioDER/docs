...
- **Add to Dataset**: Adds one example per matched thread, containing the full conversation.
...
1. Select an action to trigger when the rule is applied. Rules whose item type is **Runs** offer all of the following actions. Rules whose item type is **Threads** offer **Add to Annotation Queue**, **Add to Dataset**, and **Trigger Webhooks** only.

    - **Add to Dataset**: Add the inputs and outputs of the trace to a [dataset](/langsmith/evaluation-concepts#datasets). A thread rule instead adds one example per matched thread, containing the full conversation.
