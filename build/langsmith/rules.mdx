...
- **Action**: The form offers one action: **Add to Dataset**, **Add to Annotation Queue**, or **Trigger Webhooks**. Online evaluators, custom code evaluators, and the standalone **Extend Data Retention** action are not available for thread rules. To score a conversation, configure a [multi-turn online evaluator](/langsmith/online-evaluations-multi-turn) instead.
- **Apply to Past Runs**: Backfill is not currently offered for thread rules.

The three thread actions behave as follows:

- **Add to Dataset**: Adds one example per matched thread, holding the full conversation. A thread rule produces the same example that adding a thread by hand produces, so the behaviors listed under [Manually from a tracing project](/langsmith/manage-datasets-in-application#manually-from-a-tracing-project) apply to it as well. **Use Corrections** is not supported on thread rules.
...
1. Select an action to trigger when the rule is applied. Rules whose item type is **Runs** offer all of the following actions. Rules whose item type is **Threads** offer **Add to Dataset**, **Add to Annotation Queue**, and **Trigger Webhooks** only.

    - **Add to Dataset**: Add the inputs and outputs of the trace to a [dataset](/langsmith/evaluation-concepts#datasets). A thread rule instead adds one example per matched thread, holding the full conversation.
